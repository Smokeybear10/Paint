...
type gctx = {
  ...
  thickness: int; (** the current line thickness *)
  }
...
let draw_point (g: gctx) (p: position) : unit =
  set_graphics_state g;
  let (x, y) = ocaml_coords g p in
  (* Draw a small filled rectangle to make points more visible *)
  Graphics.fill_circle x y 2

(** Draw multiple points from a list of positions *)
let draw_points (g: gctx) (ps: position list) : unit =
  List.iter (draw_point g) ps
...
let string_of_event (g : gctx) ( e : event) : string =
  let (x,y) = event_pos e g in
  (string_of_event_type (event_type e)) ^ " @ "
     ^ (string_of_int x) ^ ","
     ^ (string_of_int y)
